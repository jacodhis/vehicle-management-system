...
class CheckoutController extends Controller
{
    ...
    public function placeOrderbyMpesa(Request $request)
    {
        $paymentId = $request->input('paymentId');
        $payerId = $request->input('PayerID');
        $pne = $request->input('phone_number');
        $amt = Cart::getSubTotal();

        $request = mpesa_request($pne,$amt,$paymentId,$payerId);

        return redirect()->back()->with('message','Order is under process');
    }
